...
| Belege (GoBD) | Rechnungen ab Ausstellung unveränderlich, Nummern lückenlos je Jahr, Datum in Nummernreihenfolge; PDF und E-Rechnung archiviert mit SHA-256, Archiv per Trigger unveränderlich; Eingangsrechnungen nur offen löschbar, jeder Schritt protokolliert, ihre Belegdatei nie einzeln | `backend/src/invoices/`, `common/numbering.ts`, `finance/payables/`, `documents/` | `invoices.int-spec.ts`, `concurrency.int-spec.ts`, `payables.int-spec.ts` |
