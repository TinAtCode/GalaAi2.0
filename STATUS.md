...
- **Nachtrag – Versicherungen und Verträge**: Neuer Reiter „Versicherungen & Verträge“ in den
  Finanzen (Recht `finance.read`).
  - **Erfasst:** Art (Versicherung, Kfz, Leasing, Miete, Telefon, Software, Energie, Wartung),
    Anbieter, Nummer, Beitrag und Rhythmus, Laufzeitende, automatische Verlängerung und
    Kündigungsfrist in Monaten.
  - **Frist:** GartenAI rechnet „kündigen bis spätestens“. Eine abgelaufene Laufzeit mit
    Verlängerung wird fortgeschrieben; ist die Frist verpasst, zählt die nächste Möglichkeit. Ab
    90 Tagen vor der Frist steht der Vertrag oben als „Kündigungsfrist naht“. Unbefristete
    Verträge sind jederzeit mit der Frist kündbar; „gekündigt am“ ergibt das Vertragsende
    (Audit-Log).
  - **Fixkosten:** Auf Wunsch läuft der Beitrag als Fixkosten mit (Liquiditätsvorschau,
    Jahresüberblick). Die Fixkosten-Zeile folgt dem Vertrag (Betrag, Rhythmus) und endet mit ihm,
    wenn er gekündigt ist oder sich nicht verlängert.
  - **Ringdiagramme:** Kosten pro Jahr nach Art und Ausgaben nach Kategorie (Jahresüberblick).
...
- **Nachtrag – Projektnummer und Lieferscheine**:
  - **Projektnummer:** Jedes neue Projekt bekommt eine fortlaufende Nummer je Jahr (P-2026-0012,
    atomar wie Angebots- und Rechnungsnummern). Bestehende Projekte wurden per Migration
    nachnummeriert. Die Nummer steht im Projektkopf und in der Liste und ist suchbar.
  - **Lieferanten:** neue Felder „unsere Kundennummer“ und Erkennungswörter für Lieferscheine
    (Firmierung, USt-IdNr. …), in den Stammdaten pflegbar.
  - **Lieferscheine:** Seite „Lieferscheine“ (Recht `document.read`). Hochladen als PDF oder Foto,
    auch vom Handy; die Texterkennung läuft im Hintergrund.
    - Danach schlägt GartenAI den Lieferanten vor, erkannt an Name, Erkennungswörtern oder der
      E-Mail-Domain.
    - Das Projekt erkennt GartenAI an der Projektnummer, der Lieferadresse oder der Kommission bzw.
      dem Bauvorhaben; dazu kommen Lieferscheinnummer und Datum.
    - Vorgeschlagen wird nur bei eindeutigem Treffer; der Grund steht dabei.
    - Das Büro bestätigt oder korrigiert, danach hängt der Lieferschein als Dokument am Projekt
      (Audit-Log). Auch am Projekt hochgeladene oder als Lieferschein erkannte Dokumente landen im
      Eingang.
  - **Lieferanten informieren:** Im Projekt unter „Lieferungen“ erzeugt GartenAI einen
    Mail-Entwurf an den gewählten Lieferanten: Projektnummer, Lieferadresse, Kommission und unsere
    Kundennummer. Er öffnet sich im Mail-Programm (`mailto:`) oder lässt sich kopieren; GartenAI
...
- **Nachtrag – Cloud-Anbindung der Mini-Vollversion** (siehe `BUERO.md`):
  - **Cloud-Sicherung:** Einrichten mit `ops/buero/cloud-setup` (rclone: Google Drive, OneDrive,
    Dropbox, Nextcloud/WebDAV, S3 …). Darüber liegt eine rclone-„crypt“-Verschlüsselung mit
    zufälligen Passwörtern: Inhalte und Dateinamen werden vor dem Hochladen verschlüsselt, die
    Passwörter stehen in `.env.buero`. Der Dienst `cloud-backup` (Profil `cloud`, startet
    automatisch, sobald eingerichtet) spiegelt stündlich `backups/buero`. Die CI prüft mit echtem
    rclone: verschlüsselt hochladen, keine lesbaren Namen oder Inhalte, Aufräumen spiegeln,
    Byte für Byte zurückholen. Zugangsdaten (`ops/buero/rclone`) sind in `.gitignore`.
  - **Unterwegs zugreifen:** Tailscale (empfohlen, nur eigene Geräte; die Tailscale-Adresse kommt
    automatisch ins Zertifikat) oder Cloudflare-Tunnel (Profil `tunnel`,
    `CLOUDFLARE_TUNNEL_TOKEN`, Ziel `http://frontend:8080`). Beides ohne offene Ports am Router.

- **Nachtrag – Anmelden mit Google** (siehe `BUERO.md`, `BETRIEB.md`):
  - Knopf „Anmelden mit Google“ auf der Login-Seite, sobald `OIDC_CLIENT_ID`, `OIDC_CLIENT_SECRET`
    und `OIDC_REDIRECT_URI` gesetzt sind; andere OpenID-Connect-Anbieter (Microsoft 365, Nextcloud,
    Keycloak) über `OIDC_ISSUER`.
  - Nur bestehende, aktive Konten, zugeordnet über die vom Anbieter bestätigte E-Mail; optional nur
    bestimmte Domains (`OIDC_ALLOWED_DOMAINS`). Neue Konten legt weiterhin das Büro an.
  - Autorisierungscode mit PKCE, State/Nonce signiert im kurzlebigen Cookie, ID-Token über JWKS
    geprüft (nur RS*/ES*). Getestet gegen einen nachgebauten Anbieter (gefälschte Tokens, fremder
    Browser, unbestätigte E-Mail, deaktivierte Konten).

- **Nachtrag – Neudesign gAla** (siehe `brand/README.md`):
  - Erscheinungsbild unter Einstellungen → Darstellung umschaltbar: **gAla** (Standard) oder
    GartenAI. gAla: Schriftzug in Navigation und Login, Tannengrün/Rasengrün, neutrale Töne mit
    Grünstich, rundere Ecken, eigener Dunkelmodus. Eigene Farben je Erscheinungsbild getrennt.
  - App-Symbol (`gala-icon.svg`, maskierbar), Manifest „gAla – Garten Landschaft Bau App“, Titel
    und Favicon folgen dem gewählten Look.

- **Nachtrag – DATEV-Debitoren:** Einstellungen → DATEV → „Debitoren (Kunden) herunterladen“
  (`GET /datev/debtors`, `?invoiced=1` nur Kunden mit Rechnungen): EXTF „Debitoren/Kreditoren“
  (Kategorie 16, Formatversion 5) mit Konto, Name (Unternehmen bzw. ohne Angabe), Kurzbezeichnung,
  USt-IdNr. getrennt nach Land und Nummer, Anschrift, Telefon, E-Mail. Fehlende Debitorennummern
  werden dabei vergeben, dieselben wie im Buchungsstapel. Die ersten 40 Spalten des Formats; die
  übrigen (Bank, Mahnwesen) bleiben leer. Beim ersten Einsatz einmal mit der Kanzlei probeweise
  importieren.
- **Nachtrag – GAEB DA XML:** Am Projekt „GAEB-Leistungsverzeichnis einlesen“ (X83, auch
  X81/X82/X86): daraus wird ein Angebotsentwurf mit Ordnungszahl, Kurztext, Menge (genau wie im LV,
  ohne Rundung) und Einheit. Heißt eine Katalog-Leistung wie der Kurztext, kommt der Preis aus der
  Kalkulation, sonst freie Position mit 0 € zum Ausfüllen. Bedarfs- und Wahlpositionen und
  Positionen ohne Menge werden gemeldet, nicht übernommen. Am Angebot „GAEB X84“: Angebotsabgabe
  mit Einheits- und Gesamtpreisen unter den Ordnungszahlen des LV, Summen je Titel; im Angebot
