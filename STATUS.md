...
- **Metriken**: `GET /metrics` im Prometheus-Format, nur mit `METRICS_TOKEN` als Bearer-Token (ohne Token abgeschaltet, 404). Anfragen je Methode, Routen-Muster (`/customers/:id`, nie echte IDs) und Status, Antwortzeiten als Histogramm, OCR-Warteschlange (laufend/wartend), fehlgeschlagene E-Mails, dazu Prozesswerte (CPU, Speicher, Event-Loop).
