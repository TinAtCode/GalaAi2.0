...
- ✅ **OCR im Server-Prozess:** Die Texterkennung lief direkt im API-Prozess; einige große Scans gleichzeitig blockierten den ganzen Server. **Stand:** Warteschlange mit begrenzter Parallelität (`OCR_CONCURRENCY`, Standard 2) für alle OCR-Aufrufe; `POST /ocr/jobs` nimmt die Datei sofort an (202), das Ergebnis kommt über `GET /ocr/jobs/:id`. Das Limit gilt inzwischen über alle Server-Instanzen (Plätze in PostgreSQL mit Lebenszeichen, ohne Redis). Offen: Ablage der Dateien im Objektspeicher.
...
| **5. Erst danach ausbauen** | Mobile App mit Offline-Sync · KI · Schnittstellen (DATEV, GAEB, DATANORM). **Stand:** DATEV-Export der Ausgangsrechnungen (Buchungsstapel EXTF), Zahlungseingänge und offene Posten, freie Angebotspositionen und Bearbeiten von Entwürfen umgesetzt; Mahnwesen (Zahlungserinnerung, 1. und 2. Mahnung, optional mit Gebühren, Verzugszinsen und Pauschale); Bankabgleich (Kontoauszug CAMT.053 einlesen, Zahlungen zuordnen); Dokumente am Projekt mit Texterkennung und Suche; Finanzbereich (Kontostände, Kontobewegungen, Forderungen, Monatsübersicht, Ausgabenkategorien mit Regeln und Lernen, Fixkosten, Jahresüberblick, Eingangsrechnungen mit E-Rechnung und Abgleich, Liquiditätsvorschau) mit Rolle Buchhaltung; Lagepläne am Projekt (Zeichnen mit Maßstab, exakten Maßen, Rundungen, Kreisen und Schächten, DXF-Import, offline auf der Baustelle, Mengen ins Angebot); Zahlungen auf Mahnkosten und Zinsen; Kontoauszüge als CAMT.053, MT940 und CSV; offen sind Mobile App, GAEB und DATANORM |
