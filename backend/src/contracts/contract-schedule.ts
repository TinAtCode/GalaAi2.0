...
import { addCalendarDays } from '../common/time-zone';
...
// Vertrag vorher, wird der Zeitraum am Vertragsende gekürzt.
export function nextBillingPeriod(
  ...
  let end = addCalendarDays(addMonths(start, MONTHS_PER_INTERVAL[contract.billingInterval]), -1);
  if (contract.endDate && end > contract.endDate) end = contract.endDate;
  return { start, end };
}
